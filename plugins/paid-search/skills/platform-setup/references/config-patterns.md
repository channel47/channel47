...
Add credentials to `.claude/settings.local.json` in your project root. This file is gitignored by default and scoped to the current project — ideal for managing multiple client accounts.
...
The plugin's `.mcp.json` uses `${VAR}` syntax (e.g., `${GOOGLE_ADS_DEVELOPER_TOKEN}`). Claude Code resolves these from the session environment, which includes both shell env vars and the `env` field from settings files.
...
| Azure app client secret | `BING_ADS_CLIENT_SECRET` |
| OAuth refresh token | `BING_ADS_REFRESH_TOKEN` |
| Manager customer ID | `BING_ADS_CUSTOMER_ID` |
| Default account ID | `BING_ADS_ACCOUNT_ID` |
