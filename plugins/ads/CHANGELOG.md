...
### Changed
- **Hooks migration:** Moved hooks from `.claude/settings.json` to `hooks/hooks.json` for proper marketplace distribution format
- **Troubleshooting skill:** Added `allowed-tools: []` restriction since it's purely reference content
...
### Changed
- **Subagent description:** Updated to use proactive trigger language ("Use proactively", "Automatically spawn") for improved invocation reliability
